...
use options::{IndexGatherCli,IndexSize};
...
fn print_am_times(cli: &IndexGatherCli,my_pe: usize, num_pes: usize, variant: &Variant, idx_size: &IndexSize, times: (Duration, Duration, Duration, Duration)){
    if my_pe == 0 {
...
fn print_array_times(cli: &IndexGatherCli,my_pe: usize, num_pes: usize, variant: &Variant, distribution: &ArrayDistribution, times: (Duration, Duration, Duration, Duration)){
    if my_pe ==0{
            ...
            match variant{
                Variant::UnsafeAM => {
                    for idx_size in &am_index_size{
                        let times  = active_message::am::index_gather(&world, &cli, &rand_index,false, idx_size) ;
                        print_am_times(&cli,my_pe,num_pes,&variant,&idx_size,times);
                    }
                }
                Variant::SafeAm => {
                    for idx_size in &am_index_size{
                        let times  = active_message::am::index_gather(&world, &cli, &rand_index,true, idx_size) ;
                        print_am_times(&cli,my_pe,num_pes,&variant,&idx_size,times);
                    }
                ...
                Variant::UnsafeBufferedAm => {
                    for idx_size in &am_index_size{
                        let times  = active_message::buffered_am::index_gather(&world, &cli, &rand_index,false, idx_size) ;
                        print_am_times(&cli,my_pe,num_pes,&variant,&idx_size,times);
                    }
                ...
                Variant::SafeBufferedAm => {
                    for idx_size in &am_index_size{
                        let times  = active_message::buffered_am::index_gather(&world, &cli, &rand_index,true, idx_size) ;
                        print_am_times(&cli,my_pe,num_pes,&variant,&idx_size,times);
                    }
                ...
                Variant::UnsafeAmGroup => {
                    for idx_size in &am_index_size{
                        let times  = active_message::am_group::index_gather(&world, &cli, &rand_index,false, idx_size) ;
                        print_am_times(&cli,my_pe,num_pes,&variant,&idx_size,times);
                    }
                ...
                Variant::SafeAmGroup => {
                    for idx_size in &am_index_size{
                        let times  = active_message::am_group::index_gather(&world, &cli, &rand_index,true, idx_size) ;
                        print_am_times(&cli,my_pe,num_pes,&variant,&idx_size,times);
                    }
                ...
                Variant::UnsafeArray => {
                    for distribution in &array_distribution{
                        let times = array::index_gather(&world, &cli, &rand_index, ArrayType::Unsafe, distribution);
                        print_array_times(&cli,my_pe,num_pes,&variant,&distribution,times);
                    }
                }
                Variant::AtomicArray => {
                    for distribution in &array_distribution{
                        let times = array::index_gather(&world, &cli, &rand_index, ArrayType::Atomic, distribution);
                        print_array_times(&cli,my_pe,num_pes,&variant,&distribution,times);
                    }
                }
                Variant::LocalLockArray => {
                    for distribution in &array_distribution{
                        let times = array::index_gather(&world, &cli, &rand_index, ArrayType::LocalLock, distribution);
                        print_array_times(&cli,my_pe,num_pes,&variant,&distribution,times);
                    }
                }
            }
