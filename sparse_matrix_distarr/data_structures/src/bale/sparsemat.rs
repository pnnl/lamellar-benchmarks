...
/// LICENSE_BALE file in the top level dirctory of the distribution.
///
...
// use rand::prelude::*;
// use rand_chacha::ChaCha8Rng;
// use rand_chacha::ChaCha8Rng::{rand::Rng, rand::RngCore};
...
// mod err;
use super::err::SparseMatError::Sparsemat;
use super::err::{ParseMmError, SparseMatError};
...
pub struct SparseMat {
    ...
    /// The nonzero column indicies; these have values between 0 and numcols
...
#[derive(Debug, Clone)]
pub struct Perm {
    /// the permutation, stored as a Vec<usize>
    pub perm: Vec<usize>,
}
...
impl Perm {
    ...
    /// get a reference to the permutation itself
    pub fn perm_ref(& self) -> & Vec<usize> {
        & self.perm
    }    
    ...
    pub fn random(n: usize, seed: u64) -> Perm {        
        let mut rng = StdRng::seed_from_u64(seed);
        let mut ret = Perm::new(n);
        ret.perm.shuffle(&mut rng );
        ret
    }
...
impl SparseMat {
    ...
    /// inspector fn for number of structural nonzeros
    pub fn nnz(&self) -> usize {
        self.nonzero.len()
    }

    /// inspector fn for offset
    pub fn offset(&self) -> &Vec<usize> {
        & self.offset
    }    

    /// inspector fn for nonzero
    pub fn nonzero(&self) -> &Vec<usize> {
        & self.nonzero
    }   
    
    /// Get a row of the matrix as `&[usize]`
    pub fn row(&self, p: usize ) -> &[usize] {
        let start = self.offset[p];
        let stop = self.offset[p +1];        
        return &self.nonzero[start..stop]
    }
    ...
    pub fn to_vec_of_rowvec(&self) -> Vec<Vec<usize>> {
        ...
        for rindex in 0 .. self.numrows {
            vecvec.push( self.row( rindex ).to_vec() );
        }
        return vecvec
    }
    ...
    pub fn erdos_renyi_tri(
        ...
        while row < numrows {
            let mut r = rng.gen::<u32>(); // rng.gen_range(0..std::u32::MAX); //
            if r == std::u32::MAX {
                r -= 1;
            }
    ...
    /// * seed A random seed.
    pub fn erdos_renyi_graph(numrows: usize, p: f64, simple: bool, seed: u64) -> SparseMat {
