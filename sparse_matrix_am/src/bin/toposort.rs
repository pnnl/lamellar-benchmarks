...
fn main() {
    ...
    for epoch in 0..num_rows_global {
        ...
        let columns_to_delete = {
            ...
            for row in rows_owned.iter() {
                if row_counts_temp[ *row ]  == 1 {
                    ...
                    diagonal_elements_temp[ epoch ].push( diagonal_element );
                }
            }
            ...
            for (row, col) in   & diagonal_elements_temp[ epoch ] {
        ...
        if **num_deleted_global.read() == num_rows_global {
            ...
            let diagonal_elements_to_move: Vec<_> = diagonal_elements.read().iter().cloned().collect();
