...
// EXAMPLE RUN COMMAND
// ```
// RUST_LIB_BACKTRACE=1 RUST_BACKTRACE=full LAMELLAR_DEADLOCK_TIMEOUT=10 LAMELLAR_THREADS=1 srun --cpus-per-task=2 --cpu-bind=ldoms,v  -N 1 --ntasks-per-node=2 -A lamellar --mpi=pmi2 --exclusive /people/roek189/learning_lamellar/lamellar_benchmarks_repo/target/release/sssp_delta_step --graph-type random --rows-per-thread-per-pe 10 --avg-nnz-per-row 8 --random-seed 0 --delta 0.3 --write-to-json
// ```
...
fn main() {
    ...
    let rows_per_thread_per_pe  =   cli.rows_per_thread_per_pe.unwrap_or(1);
    ...
    if world.my_pe() == 0 {
        ...
        println!("");
        println!("Number of epochs:                   {:?}", epoch_outer );
        println!("");                                                                                                                
