...
// use sparse_matrix_am::permutation::rand_perm;
...
//     let get_matrix_for_pe       =   | pe: usize | -> HashMap<usize, Vec<usize>> {

//         let mut permutation_row_partial_rev     =   HashMap::new();
//         for index_row in num_rows_per_pe * pe .. num_rows_per_pe * (pe+1) {
//             permutation_row_partial_rev.insert( permutation_row[index_row], index_row );
//         }
...
//                                         & permutation_row[row_owned_first_in .. row_owned_first_out],
//                                     );

//         let mut matrix              =   HashMap::new();                                    
//         for p in 0 .. indices_row.len() {
//             let index_row           =   permutation_row_partial_rev.get( & indices_row[p] ).unwrap().clone();
//             let index_col           =   permutation_col[ indices_col[p] ];
            
//             matrix.entry(index_col).or_insert(Vec::new()).push( index_row );
//         }  
...
//     let matrix                      =   get_matrix_for_pe( world.my_pe() );
...
//         // let nz_entries          =   matrix.get( column ).unwrap();
//         let nz_indices
//                   =   matrix.get( & column ).unwrap();
...
//             { 
//                 world.block_on(world.block_on(diagonal_elements_union.write())).extend( vec![vec![]; epoch+1 ] ); // ensure diagonal_elements_union is long enough (before now it had length 0) 
//             } 
...
//         println!("Number of nonzeros on PE 0:         {:?}", matrix.values().map(|x| x.len()).sum::<usize>() );                   
//         println!("");       
...
//     pub matrix:             Darc<HashMap<usize,Vec<usize>>>, 
...
//                 if let Some( nz_indices ) = self.matrix.get( column ) {
//                     for row in nz_indices {
...
//     async fn exec(self) {
//         let mut diagonal_elements_to_stay   =   self.world.block_on(world.block_on(world.block_on(diagonal_elements_to_stay.write()))).await;
