...
//------ Safe AMs -----------
#[lamellar::AmData]
struct BufferedPushDartAm {
    target: LocalRwDarc<Vec<usize>>,
    vals: Vec<usize>,
}

#[lamellar::am]
impl LamellarAM for BufferedPushDartAm {
    async fn exec(self) {
        self.target.write().await.extend_from_slice(&self.vals);
    }
}
...
impl LamellarAM for LaunchAm {
    async fn exec(self) {
        ...
        let mut buffered_darts = vec![Vec::with_capacity(self.buffer_size); lamellar::num_pes];
...
pub fn rand_perm<'a>(
    ...
    let target = target.into_darc().block();
