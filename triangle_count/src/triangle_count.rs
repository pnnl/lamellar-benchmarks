    ...
    fn exec() {
        ...
        for node_0 in (self.start..self.end).filter(|n| self.graph.node_is_local(n)) {
            task_group.exec_am_all(
                ...
                node: node_0,
                neighbors: graph_data.neighbors_iter(&node_0).take_while(|n| n < &&node_0).map(|n| *n).collect::<Vec<u32>>(), //only send neighbors that are less than node_0 as an optimization
...
impl LamellarAM for TcAm {
    fn exec() {
        let mut cnt = 0;
        for node_1 in self.neighbors.iter().filter(|n| self.graph.node_is_local(n)) { //check to make sure node_1 is local to this pe
            let neighs_1 = self.graph.neighbors_iter(node_1).take_while(|n| n < &node_1);
            cnt += TcAm::sorted_intersection_count(
                self.neighbors.iter(),
                neighs_1,
            );
        }
...
fn main(){
    ...
    if my_pe == 0 {println!("local cnt {:?}", final_cnt.load(Ordering::SeqCst))};

    if my_pe != 0 {
        world.exec_am_pe( //send the local triangle counting result to the PE 0
            0,
            CntAm {
                final_cnt: final_cnt.clone(),
    ...
    if my_pe == 0 {println!("triangles counted: {:?} global time: {:?}",final_cnt.load(Ordering::SeqCst), timer.elapsed().as_secs_f64())};
}