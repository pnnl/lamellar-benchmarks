...
impl MapGraph {
    ...
    pub fn iter(&self) -> MapGraphIter<'_>{
        MapGraphIter{iter: self.neighbors.keys()}
    }
...
impl GraphOps for MapGraph{
    fn add_local_neighbors(&mut self, node: u32, neighbors: LocalMemoryRegion<u32>)->LocalMemoryRegion<u32>{
    ...
    fn neighbors(&self, node: &u32) -> std::slice::Iter<'_, u32>{
        if let Some(n) = self.neighbors.get(node){
            match n.as_slice() {
                Ok(n) => n.iter(),
                Err(_) => panic!("node {:?} is not local to pe {:?}",node,self.team.world_pe_id()),
            }
        }
        else{
    ...
    fn node_is_local(&self, node: &u32) -> bool { //probably should abstract this out to the graphops trait
